class PaymentDataController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @payment_datum.save
        ...
        format.json { render action: 'show', status: :created, location: @payment_datum }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to payment_data_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def payment_datum_params
      params.require(:payment_datum).permit(:Client_id, :cardnumber, :cardId, :expirationdata, :securitycode)
    end
end
